...
The negative log likelihood is not differentiable with respect to $c$ because the limits of the sum are
affected by $c$.  There the gradient and hessian are derived as if $c$ is fixed.

#### Case 1 $a = c \lt b$

$$\frac{dnLL}{da} = - \frac{n}{b-a}$$

$$\frac{dnLL}{db} = \frac{n}{b-a} + \frac{n}{b-c} - \sum_i^{n} \frac{1}{b-x_i}$$

#### Case 2 $a \lt c = b$

$$\frac{dnLL}{da} = - \frac{n}{b-a} - \frac{n}{c-a} + \sum_i^{n} \frac{1}{x_i - a}$$

$$\frac{dnLL}{db} = \frac{n}{b-a}$$

#### Case 3 a \lt c \lt b

$$\frac{dnLL}{da} = - \frac{n}{b-a} - \frac{n_1}{c-a} + \sum_i^{n_1} \frac{1}{x_i - a}$$

$$\frac{dnLL}{db} = \frac{n}{b-a} + \frac{n_2}{b-c} - \sum_i^{n_2} \frac{1}{b-x_i}$$

### Hessian of the negative Log Likelihood Given $c$:

#### Case 1 $a = c \lt b$

$$\frac{d^2nLL}{da^2} = - \frac{n}{(b-a)^2}$$

$$\frac{d^2 nLL}{db^2} = -\frac{n}{(b-a)^2} - \frac{n}{(b-c)^2} + \sum_i^{n} \frac{1}{(b-x_i)^2}$$

$$\frac{d^2 nLL}{dadb} = \frac{d^2 nLL}{dbda} = - \frac{n}{(b-a)^2}$$

#### Case 2 $a \lt c = b$

$$\frac{d^2 nLL}{da^2} = - \frac{n}{(b-a)^2} - \frac{n}{(c-a)^2} + \sum_i^{n} \frac{1}{(x_i - a)^2}$$

$$\frac{d^2 nLL}{db^2} = - \frac{n}{(b-a)^2}$$

$$\frac{d^2 nLL}{dadb} = \frac{d^2 nLL}{dbda} = - \frac{n}{(b-a)^2}$$

#### Case 3 $a \lt c \lt b$

$$\frac{d^2 nLL}{da^2} = - \frac{n}{(b-a)^2} - \frac{n_1}{(c-a)^2} + \sum_i^{n_1} \frac{1}{(x_i - a)^2}$$


$$\frac{d^2 nLL}{db^2} = -\frac{n}{(b-a)^2} - \frac{n_2}{(b-c)^2} + \sum_i^{n_2} \frac{1}{(b-x_i)^2}$$
